...
把 Agent 产生的有序暂态事件，投影成 CLI stdout 的文本或 NDJSON 字节，并交付宿主课程/帮助/验收 Notice。
...
```text
Agent::run / run_debug 同步 emit(Event) + CLI Notice
  → CLI sink.try_send(OutputItem) → 64 项 mpsc → forward
      │                                  ├─ NDJSON: serialize + '\n'
      │                                  ├─ 文本: delta / 结束换行 / 调试快照与控制通知
      │                                  └─ Lab: 暂停摘要 / 工具预览 / 可选学习提示
      └─ 满/关闭 → token.cancel()             ↓
                                        Output::write_all
...
run 与 control 各持一个 Sender，`forward` 由 writer future 持有；它们通过 `join!` 并发轮询，无独立后台任务。
OutputItem::Notice 只属于 binary 的文本交付，不新增库 Event。learn 使用单 Notice 的容量 1 channel。
...
| 对象 | 创建者 / owner | 结束方式 |
|---|---|---|
| `Receiver<OutputItem>` | execute 创建，移入 forward | run/control sender 全部关闭并排空，或 forward 出错后 drop |
| 当前 Event/Notice / bytes | forward 的当前循环迭代 | 完成本项写入，或遇错丢弃剩余字节 |
...
调试快照包含全量历史，一次 Inspect 又产生一份 owned 值；调试模式没有额外快照字节限额。
重复 Inspect 或长历史会放大现有队列和序列化内存成本；这不是可靠的无限调试日志存储。
...
调试文本模式同样打印 system/messages 等完整快照，不应被误认为比 NDJSON 更安全的脱敏界面。
Lab 摘要预览截断到前 1200 个字符，但先序列化完整值；这不是字节/内存预算，也不是脱敏。
Inspect 仍输出完整快照。学习提示仅解释逻辑事实，不推导用户已掌握或 oracle 已通过。
